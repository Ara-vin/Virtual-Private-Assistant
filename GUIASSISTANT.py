...
voice_id = 0 #1 for female, 0 for male
...
def changeVoice(e):
	global voice_id
	voice_id=0
	if assVoiceOption.get()=='Male': voice_id=0
...
def main(text):
		...
		if 'whatsapp' in text:
			...
			speak('Please enter the OTP', True)
			...
			try:
				if (gen_otp == int(rec_phoneno)):
					speak('Whom do you want to send the message?', True)
					while (True):
						WAEMPOPUP("WhatsApp", "Phone Number")
						attachTOframe(rec_phoneno)
						if (rec_phoneno != '' and len(rec_phoneno) == 10):
							speak('Do you want to send a timed message?', True)
							tm_ans = record(False, False)
							if 'yes' in tm_ans:
								speak('What is the message?', True)
								message = record(False, False)
								clearChatScreen()
								speak('Mention the hour in 24-hour format', True)
								tm_hour = record(False, False)
								res = 0
								x = tm_hour.split()
								for i in x:
									if i.isnumeric():
										res=int(i)
										break
								speak('Mention the minutes', True)
								tm_min = record(False, False)
								pywhatkit.sendwhatmsg('+91' + str(rec_phoneno), str(message), res, int(tm_min))
		...
		if isContain(text, ['voice']):
			global voice_id
			try:
				if 'female' in text: voice_id = 1
				elif 'male' in text: voice_id = 0
				else:
...
def WAEMPOPUP(Service='None', rec='Reciever'):
	...
	Label(PopUProot, text=f'Receiver {rec}', font=('Arial', 16), bg='white').pack(pady=(20, 10))
...
if __name__ == '__main__':
	...
	UserFieldLBL.pack(pady=20, side=LEFT, padx=20)
	UserField = Entry(TextModeFrame, fg='white', bg='#203647', font=('Montserrat', 16), bd=10, width=18,relief=FLAT)
	UserField.place(x=10, y=24)
